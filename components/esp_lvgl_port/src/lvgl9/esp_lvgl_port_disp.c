...
lv_display_t *lvgl_port_add_disp(const lvgl_port_display_cfg_t *disp_cfg)
{
    ...
    if (disp != NULL) {
        ...
        // Validate disp_ctx
        if (disp_ctx == NULL) {
            ESP_LOGE(TAG, "disp_ctx is NULL after lv_display_get_driver_data");
            lvgl_port_unlock();
            return NULL;
        }
        ...
        // Validate io_handle from config
        if (disp_cfg->io_handle == NULL) {
            ESP_LOGE(TAG, "disp_cfg->io_handle is NULL");
            lvgl_port_unlock();
            return NULL;
        }
        
        // Validate io_handle in context
        if (disp_ctx->io_handle == NULL) {
            ESP_LOGE(TAG, "disp_ctx->io_handle is NULL (expected: %p)", (void*)disp_cfg->io_handle);
            // Try to fix it
            disp_ctx->io_handle = disp_cfg->io_handle;
        }

#if LVGL_PORT_HANDLE_FLUSH_READY
        // Validate io_handle before registering callbacks
        if (disp_ctx->io_handle != NULL) {
            const esp_lcd_panel_io_callbacks_t cbs = {
                .on_color_trans_done = lvgl_port_flush_io_ready_callback,
            };
            /* Register done callback */
            esp_err_t ret = esp_lcd_panel_io_register_event_callbacks(disp_ctx->io_handle, &cbs, disp);
            if (ret != ESP_OK) {
                ESP_LOGE(TAG, "Failed to register panel IO callbacks: %s", esp_err_to_name(ret));
            }
        } else {
            ESP_LOGW(TAG, "io_handle is NULL, skipping flush ready callback registration");
        }
#endif
