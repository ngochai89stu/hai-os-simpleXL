...
static void lvgl_port_touchpad_read(lv_indev_t *indev_drv, lv_indev_data_t *data)
{
    ...
    if (ret != ESP_OK) {
        /* Touch read failed (e.g., I2C NACK), treat as no touch */
        data->state = LV_INDEV_STATE_RELEASED;
        return;
    }

    /* Read data from touch controller */
    ret = esp_lcd_touch_get_data(touch_ctx->handle, touch_data, &touch_cnt, CONFIG_ESP_LCD_TOUCH_MAX_POINTS);
    if (ret != ESP_OK) {
        /* Touch get data failed, treat as no touch */
        data->state = LV_INDEV_STATE_RELEASED;
        return;
    }
