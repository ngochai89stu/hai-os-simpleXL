...
static void on_create(void) {
    ...
    lv_obj_set_style_text_font(enable_label, UI_FONT_MEDIUM, 0);
    lv_obj_set_style_text_color(enable_label, UI_COLOR_TEXT_PRIMARY, 0);
    
    s_enable_switch = lv_switch_create(s_content);
    lv_obj_set_style_bg_color(s_enable_switch, UI_COLOR_BG_PRESSED, LV_PART_MAIN);
    lv_obj_set_style_bg_color(s_enable_switch, UI_COLOR_PRIMARY, LV_PART_INDICATOR);
    ...
    lv_obj_set_size(s_timeout_dropdown, LV_PCT(100), UI_SIZE_BUTTON_HEIGHT);
    lv_obj_set_style_bg_color(s_timeout_dropdown, UI_COLOR_BG_SECONDARY, LV_PART_MAIN);
    lv_obj_set_style_text_font(s_timeout_dropdown, UI_FONT_MEDIUM, 0);
    
    // Image selector
    lv_obj_t *image_label = lv_label_create(s_content);
    lv_label_set_text(image_label, "Screensaver Image");
    lv_obj_set_style_text_font(image_label, UI_FONT_MEDIUM, 0);
    lv_obj_set_style_text_color(image_label, UI_COLOR_TEXT_PRIMARY, 0);
    
    s_image_selector = lv_dropdown_create(s_content);
    lv_dropdown_set_options(s_image_selector, "Default\nGradient\nImage 1\nImage 2\nCustom");
    lv_obj_set_size(s_image_selector, LV_PCT(100), UI_SIZE_BUTTON_HEIGHT);
    lv_obj_set_style_bg_color(s_image_selector, UI_COLOR_BG_SECONDARY, LV_PART_MAIN);
    lv_obj_set_style_text_font(s_image_selector, UI_FONT_MEDIUM, 0);
